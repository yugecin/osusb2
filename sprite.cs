...
squarescale:
		...
		private void fixrotation() {
			...
			if (rotcmds.First.Value.start != starttime) {
				RotCommand cmd = new RotCommand(starttime, starttime, 0f, 0f);
				addOrdened(cmd);
		...
		private void compressmove() {
			...
			LinkedListNode<MoveCommand> node = movecmds.First;
			MoveCommand first = node.Value;
			List<MoveCommand> toremove = new List<MoveCommand>();
			while ((node = node.Next) != null) {
				MoveCommand v = node.Value;
				first.to = v.to;
				first.end = v.end;
				toremove.Add(v);
			}
		...
		private void easeFloatCommandBatch<T>(LinkedList<T> cmds, LinkedList<T> originalList) where T : ICommand {
			...
			float bestscore = 1f;
			foreach (Pair<int, Eq> e in alleqs) {
				float maxdif = 0f;
				float avgdif = 0f;
