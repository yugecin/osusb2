		...
		public Ztor(int start, int stop) {
			...
			for (int a = 0; a < DIVH; a++) {
				...
				for (int b = 0; b < DIVV; b++) {
					int s = Sprite.EASE_ALL | Sprite.SESDSM | Sprite.SESDSC;
					Odot d = new Odot(Sprite.SPRITE_DOT_6_12, s);
					FadeCommand c = new FadeCommand(stop - 500, stop, 1f, 0f);
...
cull:
			...
			for (int i = 0; i < txtdots.Length; i++) {
				...
				if (isOnScreen(px)) {
					object owner = screen2.ownerAt(px);
